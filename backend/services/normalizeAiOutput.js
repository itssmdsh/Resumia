...
export function normalizeAiOutput(raw, payload) {
  ...
  return {
    company: { name: text(company.name || company.companyName || company.company_name || company.value || (typeof source.company === 'string' ? source.company : null)), confidence: confidence(company.confidence) },
